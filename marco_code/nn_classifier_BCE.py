...
class SimpleClassifier(nn.Module):
    def __init__(self, input_dim):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(64, 1)  # Output 2 classes
        )
...
def train_model(model, train_loader, val_loader, epochs):
    ...
    for epoch in range(epochs):
        ...
        for X_batch, y_batch in train_loader:
            ...
            train_correct += (predicted == y_batch.long()).sum().item() / total
        # Validate
        model.eval()
        correct = 0
        total = 0
        ...
        with torch.no_grad():
            for X_batch, y_batch in val_loader:
                ...
                total += y_batch.size(0)
                correct += (predicted == y_batch).sum().item()
                val_correct += (predicted == y_batch.long()).sum().item() / total
        ...
        if (epoch + 1) % 5 == 0:
            ...
            print(f'Train - Total:{(train_correct/len(train_loader)):.4f}')
            print(f'Validation - Total:{(val_correct /len (val_loader)):.4f}')
